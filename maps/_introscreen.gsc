...
airport_intro()
{
	...
	time = 2;
	/*lines = [];
	lines[lines.size] = "THIS";
	lines["date"] = "MAP";
	lines[lines.size] = "IS";
	lines[lines.size] = "UNFINISHED";
	lines[lines.size] = ":(";
	delayThread(2,::introscreen_feed_lines,lines);*/
	wait(time);
	wait 1;
	level notify("introscreen_complete");
	level.player FreezeControls(false);
...
trainer_intro()
{
	level.player FreezeControls( false );
	lines = [];
	lines[ lines.size ] = "Mission: Unlock All";
	lines[ lines.size ] = "Press [{+actionslot 2}] to Start!";
	introscreen_feed_lines( lines );
	level notify( "black_fading" );
